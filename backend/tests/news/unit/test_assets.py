...
def test_resolve_asset_by_explicit_id(db_session, make_instrument):
    asset = make_instrument(symbol="DEMO")
    resolution = resolve_asset(db_session, asset.id, 0.5)
    assert resolution.instrument_id == asset.id
    assert resolution.method == "explicit"
    assert resolution.confidence == 1.0


def test_resolve_asset_by_symbol_keyword(db_session, make_instrument):
    asset = make_instrument(symbol="DEMO")
...
def test_resolve_asset_unknown_symbol_is_unmatched(db_session, make_instrument):
