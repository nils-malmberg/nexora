...
from app.news.adapters.base import (
    AdapterError,
...
from app.news.adapters.registry import build_adapter
from app.news.pipeline.assets import resolve_asset
from app.news.pipeline.cache import snapshot_freshness_label
from app.news.pipeline.dedupe import find_exact_news_duplicate, find_matching_event, find_near_duplicate_news
from app.news.pipeline.retry import RetriesExhausted, call_with_retries
from app.news.pipeline.scoring import ScoreInput, compute_relevance
...
def _upsert_news_item(db: Session, run_id: str, normalized: NormalizedNewsItem, counts: dict) -> None:
    ...
    if resolution.instrument_id is not None:
        link_exists = any(link.instrument_id == resolution.instrument_id for link in item.asset_links)
        if not link_exists:
            db.add(
                NewsItemAsset(
                    news_item_id=item.id,
    ...
    score, breakdown = compute_relevance(
        ScoreInput(
            asset_match_confidence=resolution.confidence if resolution.instrument_id else 0.0,
