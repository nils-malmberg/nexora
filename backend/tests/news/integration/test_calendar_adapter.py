...
def test_fetch_parses_all_vevents():
    ...
    result = _adapter().fetch(CAL_URL, {"instrument_id": "asset-1"}, since=None)
...
def test_normalize_maps_status_and_timezone_correctly():
    ...
    adapter = _adapter()
    result = adapter.fetch(CAL_URL, {"instrument_id": "asset-1"}, since=None)
...
def test_asset_hint_falls_back_to_custom_property_when_feed_not_bound():
    ...
    adapter = _adapter()
    result = adapter.fetch(CAL_URL, {}, since=None)  # no feed-level instrument_id
