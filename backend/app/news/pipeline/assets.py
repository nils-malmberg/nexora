...
`Instrument` row, so every adapter is resolved the same way.
"""
...
def resolve_asset(db: Session, asset_hint: str | None, proposed_confidence: float) -> AssetResolution:
    ...
    if db.get(Instrument, asset_hint) is not None:
        return AssetResolution(asset_hint, "explicit", 1.0)

    asset = db.scalar(select(Instrument).where(Instrument.symbol.ilike(asset_hint), Instrument.user_id.is_(None)))
