...
def _add_feed(db_session, provider, url, instrument_id=None):
    feed = ProviderFeed(
        provider_id=provider.id, url=url, extra_config={"instrument_id": instrument_id} if instrument_id else {}
    )
    db_session.add(feed)
    db_session.commit()
...
@respx.mock
def test_run_provider_inserts_news_items(db_session, make_provider, make_instrument):
    asset = make_instrument(symbol="DEMO")
    provider = make_provider("issuer-rss", "rss")
    _add_feed(db_session, provider, RSS_URL, instrument_id=asset.id)
...
@respx.mock
def test_run_provider_is_idempotent_on_repeated_fetch(db_session, make_provider, make_instrument):
    asset = make_instrument(symbol="DEMO")
    provider = make_provider("issuer-rss", "rss")
    _add_feed(db_session, provider, RSS_URL, instrument_id=asset.id)
    respx.get(RSS_URL).mock(return_value=httpx.Response(200, content=read_fixture_bytes("rss", "valid_feed.xml")))
...
@respx.mock
def test_corroboration_relation_created_across_providers(db_session, make_provider, make_instrument):
    asset = make_instrument(symbol="DEMO")
    provider_a = make_provider("issuer-rss", "rss")
    provider_b = make_provider("media-rss", "rss")
    _add_feed(db_session, provider_a, RSS_URL, instrument_id=asset.id)
    _add_feed(db_session, provider_b, MEDIA_RSS_URL, instrument_id=asset.id)
    respx.get(RSS_URL).mock(return_value=httpx.Response(200, content=read_fixture_bytes("rss", "valid_feed.xml")))
...
@respx.mock
def test_partial_failure_isolated_per_record(db_session, make_provider, make_instrument):
    make_instrument(symbol="DEMO")
    provider = make_provider("issuer-rss", "rss")
...
@respx.mock
def test_calendar_status_change_is_traceable(db_session, make_provider, make_instrument):
    asset = make_instrument(symbol="DEMO")
    provider = make_provider("issuer-calendar", "calendar_ics")
    _add_feed(db_session, provider, CAL_URL, instrument_id=asset.id)
