...
def test_create_and_search_instrument(registered_user):
    ...
    found = client.get("/api/v1/market/search", params={"q": "micro"})
    assert found.status_code == 200
    body = found.json()
    own = [c for c in body["candidates"] if c["in_catalog"]]
    assert len(own) == 1 and own[0]["symbol"] == "MSFT" and own[0]["instrument_id"] == created.json()["id"]

    not_found = client.get("/api/v1/market/search", params={"q": "nonexistent-zzz"})
    assert not_found.json()["candidates"] == []
