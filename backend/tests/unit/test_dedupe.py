...
from app.pipeline.dedupe import find_exact_news_duplicate, find_near_duplicate_news


def _make_news_item(db_session, provider_id, asset_id, title, content_hash, publication_at):
    item = NewsItem(
...
def test_find_exact_duplicate_by_content_hash(db_session, make_provider, make_asset):
...
def test_near_duplicate_same_provider_is_duplicate_of(db_session, make_provider, make_asset):
    provider = make_provider("prov-a", "rss")
    asset = make_asset()
    when = datetime(2026, 9, 1, tzinfo=UTC)
...
def test_near_duplicate_different_provider_corroborates(db_session, make_provider, make_asset):
...
def test_dissimilar_titles_are_not_matched(db_session, make_provider, make_asset):
    provider = make_provider("prov-a", "rss")
    asset = make_asset()
    when = datetime(2026, 9, 1, tzinfo=UTC)
...
def test_outside_time_window_is_not_matched(db_session, make_provider, make_asset):
