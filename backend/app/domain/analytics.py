...
from app.domain.fx import prepare_fx
...
from app.models import Instrument, OhlcBar, Portfolio, PricePoint, Transaction
...
def compute_positions_as_of(db: Session, portfolio: Portfolio, as_of: datetime) -> list[PositionView]:
    ...
    for instrument_id in sorted(instrument_ids):
        ...
        view = build_position_view(db, portfolio, instrument, lots, as_of=as_of)
        if view is not None:
            views.append(view)
    views.sort(key=lambda v: v.instrument.symbol)
    return views
...
def compute_valuation_as_of(db: Session, portfolio: Portfolio, as_of: datetime) -> ValuationView:
    ...
    return aggregate_valuation(db, portfolio, positions, cash_by_currency, as_of=as_of)
...
def _candidate_dates(db: Session, portfolio_id: str, start: datetime, end: datetime) -> list[datetime]:
    ...
    if instrument_ids:
        ...
        dates.update(
            db.scalars(
                select(OhlcBar.as_of).where(
                    OhlcBar.instrument_id.in_(instrument_ids), OhlcBar.as_of >= start, OhlcBar.as_of <= end
                )
            )
        )
    dates.add(start)
...
def _portfolio_currencies(db: Session, portfolio_id: str) -> set[str]:
    currencies = set(
        db.scalars(select(Transaction.currency).where(Transaction.portfolio_id == portfolio_id).distinct())
    )
    currencies.update(
        db.scalars(
...
def valuation_history(
    db: Session, portfolio: Portfolio, start: datetime, end: datetime, *, max_points: int = 400
) -> list[ValuationPoint]:
    prepare_fx(db, _portfolio_currencies(db, portfolio.id), portfolio.base_currency, start, end)
    dates = _candidate_dates(db, portfolio.id, start, end)
    if len(dates) > max_points:
        # Daily bars over years produce thousands of candidate dates: thin
        # them evenly (always keeping both ends) rather than replaying every
        # single day on each request.
        step = len(dates) / (max_points - 1)
        dates = [dates[int(i * step)] for i in range(max_points - 1)] + [dates[-1]]
    points = []
    for as_of in dates:
...
def allocation_breakdown(db: Session, portfolio: Portfolio, positions: list[PositionView]) -> AllocationView:
    ...
    for position in positions:
        ...
        if value is None:
            ...
            continue
        by_class[position.instrument.asset_class] = by_class.get(position.instrument.asset_class, Decimal("0")) + value
        by_instrument[position.instrument.symbol] = by_instrument.get(position.instrument.symbol, Decimal("0")) + value
        by_currency[position.currency] = by_currency.get(position.currency, Decimal("0")) + value
        base_total += value
