...
def test_position_in_foreign_currency_is_converted_with_rate_provenance(registered_user):
    client, csrf, _user_id = registered_user
    ...
    valuation = client.get(f"/api/v1/portfolios/{portfolio_id}/valuation").json()
    # 10 x 110 USD = 1100 USD x 0.92 (fixture ECB-style reference rate) = 1012 EUR
    assert valuation["positions_value"] == "1012.000000"
    assert valuation["unconverted_currencies"] == []
    assert valuation["fx_rates"][0]["currency"] == "USD"
    assert Decimal(valuation["fx_rates"][0]["rate"]) == Decimal("0.92")
    assert valuation["fx_rates"][0]["source"] == "fixture-fx"
    position = valuation["positions"][0]
    assert position["market_value"] == "1100.000000"
    assert position["market_value_base"] == "1012.000000"
    assert Decimal(position["fx_rate"]) == Decimal("0.92")
    assert position["unrealized_pnl"] == "100.000000"
    # the USD purchase itself left a negative USD cash balance, converted too
    assert valuation["cash_by_currency"]["USD"] == "-1000.000000"
    assert valuation["cash"] == "-920.000000"


def test_position_in_unknown_currency_is_excluded_not_invented(registered_user):
    client, csrf, _user_id = registered_user
    portfolio_id = _make_portfolio(client, csrf, base_currency="EUR")
    instrument_id = _make_instrument(client, csrf, symbol="CHF-STOCK", currency="CHF")
    client.post(
        f"/api/v1/portfolios/{portfolio_id}/transactions",
        json={
            "instrument_id": instrument_id,
    ...
    client.post(
        f"/api/v1/instruments/{instrument_id}/prices",
        json={"as_of": "2026-01-02T00:00:00Z", "price": "110", "currency": "CHF"},
        headers=_headers(csrf),
    )
    valuation = client.get(f"/api/v1/portfolios/{portfolio_id}/valuation").json()
    assert valuation["positions_value"] == "0.000000"
    assert valuation["unconverted_currencies"] == ["CHF"]
    assert valuation["positions"][0]["market_value_base"] is None
