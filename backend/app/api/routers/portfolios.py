...
def _position_out(view: PositionView) -> PositionOut:
    ...
    return PositionOut(
        ...
        price_source=view.price.source if view.price else None,
        market_value=view.market_value,
        market_value_base=view.market_value_base,
        unrealized_pnl=view.unrealized_pnl,
        freshness=view.freshness,
        matches_base_currency=view.matches_base_currency,
        fx_rate=conv.rate if conv else None,
        fx_rate_as_of=conv.rate_as_of if conv else None,
...
def _validate_instrument(db: Session, user: User, instrument_id: str | None) -> Instrument | None:
    if instrument_id is None:
        return None
    return get_visible_instrument(instrument_id, db, user)
...
def get_valuation(
    ...
    return ValuationOut(
        ...
        fx_rates=[
            FxNoteOut(currency=n.currency, rate=n.rate, rate_as_of=n.rate_as_of, source=n.source)
            for n in valuation.fx_rates
        ],
