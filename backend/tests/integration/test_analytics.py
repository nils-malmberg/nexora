...
def test_allocation_converts_known_currency_and_lists_unknown_separately(registered_user):
    """USD has a (fixture) reference rate -> converted, with provenance;
    CHF has none -> excluded from the total and listed, never converted at
    an invented rate."""
    client, csrf, _user_id = registered_user
    portfolio_id = _make_portfolio(client, csrf)
    for symbol, currency in (("USDX", "USD"), ("CHFX", "CHF")):
        instrument = client.post(
            "/api/v1/instruments",
            json={"symbol": symbol, "name": f"{currency} co", "asset_class": "action", "currency": currency},
            headers=_headers(csrf),
        ).json()
        client.post(
        ...
        client.post(
            ...
            json={"as_of": "2026-01-02T00:00:00Z", "price": "100", "currency": currency},
            headers=_headers(csrf),
        )

    body = client.get(f"/api/v1/portfolios/{portfolio_id}/analytics/allocation").json()
    assert body["total_value"] == "920.000000"  # 10 x 100 USD x 0.92
    assert body["unconverted_currencies"] == ["CHF"]
    assert [s["label"] for s in body["by_currency"]] == ["USD"]
